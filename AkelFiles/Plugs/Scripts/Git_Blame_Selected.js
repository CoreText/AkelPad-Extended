...
// will show you log and you'll able to navigate through the lines
//
// Arguments:
// 	"cmd" - command to run
// 	"%f"  - current file
...
if (hWndEdit = AkelPad.GetEditWnd())
{
	...
	if (arg)
	{
		...
		var pCommand = AkelPad.GetArgValue(
			"cmd",
			"cmd.exe /K cd /d \"" + dir + "\"  & echo file path: \"" + currFullPathFile + "\" & git blame -L " + (nSelStart + 1) + "," + (nSelEnd + 1) + " \"" + currFile + "\" & exit "
		);
		...
		AkelPad.Call("Log::Output", 1, pCommand, "", "^([\\^\\w\\d]{0,8})\\s(\\(.+?(\\d)*\\))\\s", "/GOTOLINE=\\3:0", -1, -1, 50, ".git");
		pCommand = null;
	}
